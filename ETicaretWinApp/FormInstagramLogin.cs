...
{
    public partial class FormInstagramLogin : Form
    {
        ...
        public FormInstagramLogin()
        {
            ...
            txtUser.Text = ApplicationSettingHelper.ReadValue("Instagram", "UserName");
            txtPass.Text = ApplicationSettingHelper.ReadValue("Instagram", "Password");
        ...
        private async void LoginButton_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                ApplicationSettingHelper.AddValue("Instagram", "UserName", txtUser.Text);
                ApplicationSettingHelper.AddValue("Instagram", "Password", txtPass.Text);
               
                if (checkBoxGetFallowers.Checked)
                {
                    var userData = InstaApi.GetLoggedUser();
                    var followers = await InstaApi.UserProcessor.GetUserFollowersByIdAsync(userData.LoggedInUser.Pk, PaginationParameters.MaxPagesToLoad(10));
                    if (followers.Succeeded)
                    {
                        foreach (var fallow in followers.Value)
                        {
                            InstagramUserFollowing instagramUser = new InstagramUserFollowing()
                            {
                                Fallower=true,
                                Fallowing=false,
                                ProfilePicture=fallow.ProfilePicture,
                                UserName=fallow.UserName,
                                UserPK=fallow.Pk.ToString()
                            };
                            ApiHelper.SaveInstagramUser(instagramUser);
                        }
                    }
                    var fallowingUsers = await InstaApi.UserProcessor.GetUserFollowingByIdAsync(userData.LoggedInUser.Pk, PaginationParameters.MaxPagesToLoad(10));
                    if (fallowingUsers.Succeeded)
                    {
                        foreach (var fallow in fallowingUsers.Value)
                        {
                            InstagramUserFollowing instagramUser = new InstagramUserFollowing()
                            {
                                Fallower = false,
        ...
        private async void buttonSharePicture_Click(object sender, EventArgs e)
        {
